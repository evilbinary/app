...
void test_time() {
  ...
  // 打印时间（秒和纳秒）
  printf("Seconds: %lld, Nanoseconds: %d\n", ts.tv_sec, ts.tv_nsec);

  // 检查秒和纳秒的合理性
  assert_true(ts.tv_sec > 0);
  // assert_in_range(ts.tv_nsec, 0, 999999999);

  result = clock_gettime(CLOCK_MONOTONIC, &start_ts);
  assert_int_equal(result, 0);
  printf("start_ts Seconds: %lld, Nanoseconds: %d\n", start_ts.tv_sec,
         start_ts.tv_nsec);
  ...
  printf("now Seconds: %lld, Nanoseconds: %d\n", now.tv_sec, now.tv_nsec);
